...
module UsersHelper
	...
	def get_directions(street_orig,street_dest)
		street_orig_p = street_orig + " Ottawa"
		street_dest_p = street_dest + " Ottawa"

		street_orig_uri = URI.escape(street_orig_p)
		street_dest_uri = URI.escape(street_dest_p)
		query_forward = JSON.parse(open("http://maps.googleapis.com/maps/api/directions/json?origin=#{street_orig_uri}&destination=#{street_dest_uri}&sensor=false").read)
		query_back = JSON.parse(open("http://maps.googleapis.com/maps/api/directions/json?origin=#{street_dest_uri}&destination=#{street_orig_uri}&sensor=false").read)
		...
		result = []
		if query_forward['routes'][0].nil?
			result
		else
			query_forward['routes'][0]['legs'][0]['steps'].each do |step|
			...
			query_back['routes'][0]['legs'][0]['steps'].each do |step|
				check = strip_tags(step.fetch('html_instructions'))
				roads.each do |road|
					if check.include?(road)
						result.push(road)
